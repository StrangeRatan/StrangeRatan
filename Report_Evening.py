...
def main():
    ...
    # Set the desired date and time (one day before, 11 PM)
    specified_time = datetime.now() - timedelta(days=1)
    specified_time = specified_time.replace(hour=23, minute=0, second=0, microsecond=0)
    formatted_date_time = specified_time.strftime("%d-%b-%Y_%I-%M-%S%p")

    # Create the output string
    output = f"Date/Time\tEvening {formatted_date_time}\n" \
             f"Number of store aduit\t{total_images_folder1 + total_images_folder2}\n" \
             f"Store Open at 11:00 AM\t{total_images_folder1}\n" \
             f"Store Close at 11:00 AM\t{total_images_folder2}\n" \
    ...
    # Save the workbook to an Excel file in the 'Result' folder with the current date and time as the filename
